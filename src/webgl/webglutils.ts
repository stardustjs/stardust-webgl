...
export function compileProgram(GL: WebGLRenderingContext, vsCode: string, fsCode: string): WebGLProgram {
    // Vertex shader
    ...
    if(!success) {
        ...
        throw new RuntimeError(`could not compile vertex shader: ${GL.getShaderInfoLog(vsShader)}`);
    }
    // Fragment shader
    ...
    if(!success) {
        ...
        throw new RuntimeError(`could not compile fragment shader: ${GL.getShaderInfoLog(fsShader)}`);
    }
    // Link the program
